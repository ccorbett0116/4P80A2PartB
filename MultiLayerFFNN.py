import numpy as np
import pickle
from Neuron import Neuron, derivative
from tqdm import tqdm
import matplotlib.pyplot as plt

class MLFFNN():
    def __init__(self, input_size=None, hidden_layers=None, hidden_sizes=None, output_size=None, input_data=None,
                 labels=None, num_epochs=None, learning_rate=None, loss="MSE", features_test=None, labels_test=None,
        ...
        if savedNetwork:
            ...
            self.hidden_layers = [[Neuron() for _ in range(hidden_sizes[i])] for i in range(hidden_layers)]
    ...
    def backpropagate(self, input_set, hidden_outputs, y_true, y_pred, learning_rate):
        # Compute gradients for output layer
        dL_dz_output = y_pred - y_true  # Step 1: Error calculated at the output layer
        dL_dW = [np.outer(hidden_outputs[-1], dL_dz_output)]  # Compute dL_dW for the output layer
        # The outer product works here because each weight update for the output layer depends on
        # both the hidden neuron activation and the error gradient at the output neuron.
        # The outer product allows us to efficiently compute all the weight updates in one step,
        # applying the chain rule: (dError/dOutput) * (dOutput/dWeight) for every combination of neurons.
        ...
        for i in range(len(self.hidden_layers) - 1, -1, -1):
            ...
            # Backpropagate the error to the current hidden layer by multiplying the error gradient (dL_dz)
            # with the weight matrix (self.W[i + 1]). The dot product efficiently computes the sum of the
            # contributions of all neurons in the next layer (i+1) to each neuron in the current layer (i).
            # This captures how much each neuron in layer i contributed to the error in layer i+1, enabling
            # the appropriate scaling of gradients for updating weights in the current layer.


            # Get the previous output (or input) for the current hidden layer
            ...
            for j, neuron in enumerate(self.hidden_layers[i]):
                neuron.bias -= learning_rate * dL_dz[j]  # Ensure dL_dz has the correct size for each hidden layer

        # Update weights
        for i in range(len(self.W)):
            self.W[i] -= learning_rate * dL_dW[i]

        # Update biases for the output neurons
        for i, neuron in enumerate(self.output_neurons):
            neuron.bias -= learning_rate * dL_dz_output[i]
    ...
    def train(self, networkNumber):
        ...
        for epoch in epoch_progress:
            ...
            for i in range(len(self.input_data)):
                ...
                #dont backprop on last epoch
                if epoch == self.num_epochs - 1:
                    continue
                self.backpropagate(input_set, hidden_outputs, y_true, y_pred, self.learning_rate)
            test_correct = 0
            test_incorrect = 0
